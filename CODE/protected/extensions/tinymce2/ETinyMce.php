   ...
   *
   ...
   protected function makeCompressor()
   {
      ...
      if ($this->useElFinder) {
          $options['file_browser_callback'] = 'js:function(field_name, url, type, win) {
            ...
            if($("#finder").length == 0) {
                ...
                $("#finder").elfinder({ zIndex: 9999999, places: "Favorite", url : "'.Yii::app()->urlManager->createUrl('/elfinder/default/connector').'",
                ...
                //$("#finder").elfinder({ zIndex: 9999999, places: "Favorite", url : "/elfinder/connectors/php/connector.php", 
                    lang : "en",
                    dialog : { width : 900, modal : true, title : "File Manager", zIndex: 777776 },
	...
	public function init()
	{
      ...
      $assets2 = Yii::app()->getAssetManager()->publish($baseDir.DIRECTORY_SEPARATOR.'assets');
	  //$assets2=Yii::app()->request->getBaseUrl(true)."/tinymce";
      $tinyOptions = $this->makeOptions($assets2);
      ...
      if ($this->useCompression) {
         $cs->registerScriptFile($assets2.'/tiny_mce/tiny_mce_gzip.js');
...
EOP;
         ...
         $cs->registerScriptFile($assets2.'/tiny_mce/tiny_mce.js');
      }

      if ($this->useElFinder) {
         $cs->registerScriptFile($assets2.'/elfinder/js/jquery-ui-1.7.2.custom.min.js');
         $cs->registerScriptFile($assets2.'/elfinder/js/elfinder.min.js');
         $cs->registerScriptFile($assets2.'/elfinder/js/i18n/elfinder.uk.js');
         $cs->registerCssFile($assets2.'/elfinder/js/ui-themes/base/ui.all.css');
         $cs->registerCssFile($assets2.'/elfinder/css/elfinder.css');
      }

      $cs->registerScriptFile($assets2.'/jquery/jquery.tinymce.js');
      $cs->registerScriptFile($assets2.'/embedmedia/embed.js');
