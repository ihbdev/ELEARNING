<?php
    ...
    foreach (array_diff(explode(',',$model->$attribute),array('')) as $image_id){
    	...
    	if(isset($image))
    		echo '<div class="item-image" id="'.$image_id.'"><img style="height:'.$h.'px; width:'.$w.'px" src="'.$image->getThumb($category,$type_image).'" /><a target="_blank" class="edit" href="'.Yii::app()->createUrl('image/update',array('id'=>$image_id)).'" onclick="image_select=$(this);update_form_image();return false;"></a><a class="close"></a></div>';
    }
    ?>
    </div>
...
<!-- For Display Popup to Update Image -->
    <div id="popUpDiv" style="z-index:10000;display: none;">
    	<div class="sendMarkOutline" style="border:2px solid #21629B;border-radius:8px;">
	        <div class="sendMark">
	            <a style= "float:right;margin-top:-20px" onclick="popup('popUpDiv')"><img src="<?php echo Yii::app()->request->getBaseUrl(true)?>/images/close.png"></a>
